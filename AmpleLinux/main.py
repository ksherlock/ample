...
from mame_downloader import VgmModDownloadWorker

# Linux: no winreg needed
winreg = None

def _xdg_open(path):
    """Cross-platform open: uses xdg-open on Linux."""
    try:
...
def _detect_linux_dark_mode():
    ...
    try:
        ...
        pass
    # Try GNOME / GTK gtk-theme (older GNOME)
    try:
        result = subprocess.run(
...
class RomManagerDialog(QDialog):
    ...
    def open_roms_folder(self):
        _xdg_open(self.rom_manager.roms_dir)
...
class AmpleMainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Ample - Linux Port")
        ...
        mame_exe = os.path.join(mame_bin_dir, "mame")
    ...
    def update_command_line(self):
        ...
        if hasattr(self, 'vgm_check') and self.vgm_check.isChecked():
            mame_bin_dir = os.path.dirname(self.launcher.mame_path)
            if os.path.exists(os.path.join(mame_bin_dir, "mame-vgm")):
    ...
    def show_settings(self):
        ...
        # Linux: No auto-download. Show guidance instead.
        hint_label = QLabel(
            "💡 Install MAME via your package manager:\n"
            "  Ubuntu/Debian: sudo apt install mame\n"
            "  Fedora: sudo dnf install mame\n"
            "  Arch: sudo pacman -S mame\n"
            "  Flatpak: flatpak install flathub org.mamedev.MAME"
        )
        hint_label.setWordWrap(True)
        hint_label.setStyleSheet("font-size: 11px; color: #888; padding: 8px;")
    ...
    def select_mame(self, dialog, label):
        path, _ = QFileDialog.getOpenFileName(dialog, "Select MAME", "/usr/bin", "All Files (*)")
        if path:
            self.launcher.mame_path = path
            self.check_for_mame(label)

    # Linux: No download_mame or on_mame_dl_finished needed.
    # Users install MAME via their system package manager.

    def check_for_mame(self, label=None):
        """Helper to check standard paths and update UI."""
        potential_paths = [
            os.path.join(self.app_dir, "mame", "mame"),
            os.path.join(self.app_dir, "mame"),
        ]
        
        # Linux: also check common system-wide install paths
        for sys_path in ['/usr/bin/mame', '/usr/games/mame', '/usr/local/bin/mame']:
            potential_paths.append(sys_path)
        
        # Also try to find mame via PATH (which)
        try:
            result = subprocess.run(['which', 'mame'], capture_output=True, text=True, timeout=2)
            which_path = result.stdout.strip()
            if which_path and os.path.isfile(which_path):
                potential_paths.insert(0, which_path)
        except Exception:
            pass
    ...
    def launch_mame(self):
        ...
        import shlex
        try:
            # posix=True is correct for Linux (handles quoting properly)
        ...
        try:
            ...
            exe_cmd = os.path.basename(args[0]).lower()
            vgm_exe = None
    ...
    def on_vgm_check_changed(self, state):
        if state == Qt.Checked.value:
            mame_bin_dir = os.path.dirname(self.launcher.mame_path)
            vgm_exe = os.path.join(mame_bin_dir, "mame-vgm")
    ...
    def on_vgm_dl_finished(self, worker, success, path, label, dialog):
        ...
        if success:
            QMessageBox.information(self, "Success", "MAME VGM Mod has been installed as mame-vgm")
            dialog.accept()
        else:
